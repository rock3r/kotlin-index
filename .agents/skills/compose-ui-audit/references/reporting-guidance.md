...
- Flow operators and `stateIn`/`shareIn` owned by presenters/coordinators rather than composable bodies
- local hover/focus/text-field/expanded state that is purely visual
- focus requested from effects keyed to target visibility rather than from the composable body
