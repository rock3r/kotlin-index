...
using the configured reasoning model.
"""
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for an agent skill called "{skill_name}". A skill is reusable operational guidance with progressive disclosure: the agent sees a title and description when deciding whether to use the skill, then reads the skill file and any referenced resources only after selecting it.

The description appears in the agent's available-skills list. When a user sends a query, the agent decides whether to invoke the skill based on the title and description. Your goal is to write a description that triggers for relevant queries and does not trigger for irrelevant ones.
