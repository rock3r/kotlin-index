...
Read this when creating or editing a bundled/exported Compose Pi skill, or when turning a repo-local workflow into a skill that may be exported to another project.

## Compose Pi skill classes

Compose Pi uses `compose-pi-skills-bundle.json` to distinguish two classes:

- `include`: bundled/exported skills. These ship with Compose Pi and can be exported into other projects, so treat them as portable skills.
- `exclude`: non-bundled repo-local maintainer skills. These can reference Compose Pi repository docs, local validation commands, and maintainer-only workflows.

A product/domain-specific bundled skill may still name Compose Pi, Jewel, Spectre, or another product when that product is the domain of the task. The audit is about avoiding accidental harness or checkout assumptions.
